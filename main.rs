...
impl Explosion {
    fn new(x: f32, y: f32) -> Self { Self { x, y, life: 0.6, size: 8.0 } }
}
    ...
    let shot_sound = make_shot_sound().await;
        ...
        loop {
                    ...
                    if skill_cooldown <= 0.0 {
                        ...
                        for z in zombies.into_iter() {
                            ...
                            if z.hp <= 0.0 {
                                ...
                                explosions.push(Explosion::new(z.x, z.y));
                                // grant exp
            ...
            for ex in explosions.iter_mut() {
                ex.life -= dt;
                ex.size += 60.0 * dt;
                let alpha = (ex.life / 0.6).max(0.0);
                draw_circle(ex.x, ex.y, ex.size, Color::new(1.0, 0.5, 0.0, alpha));
